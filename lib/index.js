...
server.route({ method: 'GET', path: '/profile/network', config: { handler: Profile.getDefaultNetworkProfile }});
   ...
   config: {
      validate: {
         payload: {
            ...
            bootdev: Joi.string().required(),
            protocol: Joi.string().required(),
            dns: Joi.array().required()
         }
      },
      handler: Profile.postAPINetworkProfile
   }
});

var bootstrap = { handler: { directory: { listing: false, index: false, path: './content/bower_components/bootstrap' } } };
var js = { handler: { directory: { listing: false, index: false, path: './content/bower_components/bootstrap/js' } } };
server.route({ method: 'GET', path: '/css/{path*}', config: bootstrap });
