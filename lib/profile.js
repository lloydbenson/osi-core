...
exports.getAPIHostProfile = function(request,reply) {
   var file=appHome+"/save/profile/network/"+request.params.name+".json";
...
exports.postAPINetworkProfile = function(request,reply) {
   var file=appHome+"/save/profile/network/"+request.payload.name+".json";
   Fs.writeFileSync(file,JSON.stringify(request.payload,null,3));
   reply('');
};
